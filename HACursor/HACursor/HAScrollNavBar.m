...
- (void)buttonMoveAnimationWithIndex:(NSInteger)index{
    ...
    if (self.tmpKeys.count * self.itemW > self.width) {
        ...
        CGFloat contentSize = selectButton.center.x - self.center.x;
        if (contentSize>self.contentSize.width-self.width) {
            contentSize = self.contentSize.width-self.width;
        } else if (contentSize<=0){
            contentSize = 0;
        }
        [self setContentOffset:CGPointMake(contentSize, 0)  animated:YES];
